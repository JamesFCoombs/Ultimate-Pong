...
public class Ball
{
  ...
  */}
  ...
  void test_collision(Ball other) {
    if (4 * Ball.RADIUS * Ball.RADIUS <= (xPos - other.getXPos()) * (xPos - other.getXPos()) + (yPos - other.getYPos()) * (yPos - other.getYPos())) 
     return;
    ...
    int vx = this.getXVel();
    int vy = this.getYVel();
    int bx = other.getXPos();
    int by = other.getYPos();
    int ax = xPos;
    int ay = yPos;
    int x = bx-ax;
    int y = by-ay;
    ...
    x = x / (int)Math.sqrt(x*x + y*y);
    y = y / (int)Math.sqrt(x*x + y*y);
    
    int xchange = x*(vx*x + vy*y)/(x*x+y*y);
    int ychange = y*(vx*x + vy*y)/(x*x+y*y);
    
    other.updateXV(xchange);
    other.updateYV(ychange);
    this.updateXV(-1*xchange);
    this.updateYV(-1*ychange);
  }
  
  void updateXV(int change) {
    XVelChange += change;
  }
  
  void updateYV(int change) {
    YVelChange += change;
  }
}
